...
router.delete('/:id', async (req, res) => {
  try {
    const { rows } = await pool.query(
      'UPDATE aet_projetos SET ativo = FALSE WHERE id=$1 AND ativo RETURNING id, nome_empresa',
      [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Projeto AET não encontrado' });
    await registrarAuditoria(req, 'EXCLUSÃO', 'aet_projetos', req.params.id, `Projeto AET desativado: ${rows[0].nome_empresa ?? ''}`);
    res.status(204).end();
  } catch (err) {

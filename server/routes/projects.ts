...
router.get('/', async (_req, res) => {
  try {
    const { rows } = await pool.query('SELECT dados FROM aet_projetos ORDER BY criado_em');
...
router.post('/', async (req, res) => {
  ...
  try {
    await pool.query(
      `INSERT INTO aet_projetos (id, dados) VALUES ($1, $2)
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const { rowCount } = await pool.query(
      `UPDATE aet_projetos SET dados=$1, atualizado_em=NOW() WHERE id=$2`,

...
-- ------------------------------------------------------------
CREATE TABLE medicoes_iluminancia (
  id                UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  -- Referência: avaliacao_aep_id OU funcao_id (apenas um preenchido)
  avaliacao_aep_id  UUID        REFERENCES aep_avaliacoes(id) ON DELETE CASCADE,
...
-- ------------------------------------------------------------
CREATE TABLE medicao_iluminancia_linhas (
  id          UUID  PRIMARY KEY DEFAULT gen_random_uuid(),
  medicao_id  UUID  NOT NULL REFERENCES medicoes_iluminancia(id) ON DELETE CASCADE,
...
CREATE TABLE medicao_linha_valores (
  id             UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
  linha_id       UUID          NOT NULL REFERENCES medicao_iluminancia_linhas(id) ON DELETE CASCADE,
...
-- Representação alternativa (linha × coluna → lux)
-- ------------------------------------------------------------
CREATE TABLE medicao_pontos_grade (
  id            UUID  PRIMARY KEY DEFAULT gen_random_uuid(),
  medicao_id    UUID  NOT NULL REFERENCES medicoes_iluminancia(id) ON DELETE CASCADE,
  indice_linha  INT   NOT NULL,
  indice_coluna INT   NOT NULL,
...
-- Médias por linha de medição (rowAverages do IlluminanceCalculationResult)
CREATE TABLE medicao_linha_medias (
  id          UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
  medicao_id  UUID          NOT NULL REFERENCES medicoes_iluminancia(id) ON DELETE CASCADE,
  tipo_linha  TEXT          NOT NULL CHECK (tipo_linha IN ('r','q','p','t')),
  indice_linha INT          NOT NULL,
...
-- 13 perguntas pré-definidas — respondidas por medição
-- ------------------------------------------------------------
CREATE TABLE medicao_verificacao_itens (
  id          UUID  PRIMARY KEY DEFAULT gen_random_uuid(),
  medicao_id  UUID  NOT NULL REFERENCES medicoes_iluminancia(id) ON DELETE CASCADE,
...
-- 9 tipos de inconsistências pré-definidas
-- ------------------------------------------------------------
CREATE TABLE medicao_inconsistencia_itens (
  id          UUID     PRIMARY KEY DEFAULT gen_random_uuid(),
  medicao_id  UUID     NOT NULL REFERENCES medicoes_iluminancia(id) ON DELETE CASCADE,
...
-- Índices
-- ------------------------------------------------------------
CREATE INDEX idx_medicoes_aep_aval       ON medicoes_iluminancia(avaliacao_aep_id);
CREATE INDEX idx_medicoes_param_norm     ON medicoes_iluminancia(parametro_normativo_id);
CREATE INDEX idx_medicoes_funcao        ON medicoes_iluminancia(funcao_id);
CREATE INDEX idx_med_linhas_medicao     ON medicao_iluminancia_linhas(medicao_id);
CREATE INDEX idx_med_linha_vals_linha   ON medicao_linha_valores(linha_id);
CREATE INDEX idx_med_pontos_grade       ON medicao_pontos_grade(medicao_id);
CREATE INDEX idx_med_verificacao_meds   ON medicao_verificacao_itens(medicao_id);
CREATE INDEX idx_med_inconsistencia     ON medicao_inconsistencia_itens(medicao_id);

...
-- ATENÇÃO: execute APÓS 003_create_projects.sql
-- ============================================================
...
-- ------------------------------------------------------------
CREATE TABLE aep_registros_fotograficos (
  id            UUID  PRIMARY KEY DEFAULT gen_random_uuid(),
  avaliacao_id  UUID  NOT NULL REFERENCES aep_avaliacoes(id) ON DELETE CASCADE,
...
-- categoria define qual lista:
--   postura_e_alcance | repetitividade_e_ritmo | forca_e_demanda_fisica |
--   manuseio_manual_materiais | moveis_e_posto
-- ------------------------------------------------------------
CREATE TABLE aep_itens_biomecanica (
  id                    UUID  PRIMARY KEY DEFAULT gen_random_uuid(),
  avaliacao_id          UUID  NOT NULL REFERENCES aep_avaliacoes(id) ON DELETE CASCADE,
...
-- ------------------------------------------------------------
CREATE TABLE aep_ferramentas_cientificas (
  id           UUID  PRIMARY KEY DEFAULT gen_random_uuid(),
  avaliacao_id UUID  NOT NULL REFERENCES aep_avaliacoes(id) ON DELETE CASCADE,
...
-- ------------------------------------------------------------
CREATE TABLE aep_respostas_psicossocial (
  id             UUID  PRIMARY KEY DEFAULT gen_random_uuid(),
  avaliacao_id   UUID  NOT NULL REFERENCES aep_avaliacoes(id) ON DELETE CASCADE,
...
-- Índices
-- ------------------------------------------------------------
CREATE INDEX idx_aep_avaliacoes_funcao      ON aep_avaliacoes(funcao_id);
CREATE INDEX idx_aep_registros_avaliacao    ON aep_registros_fotograficos(avaliacao_id);
CREATE INDEX idx_aep_biomecanica_avaliacao  ON aep_itens_biomecanica(avaliacao_id);
CREATE INDEX idx_aep_ferramentas_avaliacao  ON aep_ferramentas_cientificas(avaliacao_id);
CREATE INDEX idx_aep_ferramentas_modelo     ON aep_ferramentas_cientificas(modelo_id);
CREATE INDEX idx_aep_raci_fator_biom        ON aep_acoes_raci(fator_risco_biomecanico_id);
CREATE INDEX idx_aep_psicossocial_avaliacao ON aep_respostas_psicossocial(avaliacao_id);
CREATE INDEX idx_aep_gatilhos_avaliacao     ON aep_gatilhos_aet(avaliacao_id);
CREATE INDEX idx_aep_raci_avaliacao         ON aep_acoes_raci(avaliacao_id);
...
ALTER TABLE medicoes_iluminancia
  ...
  FOREIGN KEY (avaliacao_aep_id)
  REFERENCES aep_avaliacoes(id)
  ON DELETE CASCADE;
